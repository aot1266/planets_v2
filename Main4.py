...
class liquid_point:
    def __init__(self, cart_pos):
        ...
        self.g_force = np.zeros(2)
        self.g_force_abs = 0.0

class liquid:
    def __init__(self, start_arc, end_arc, depth, planet):
        self.depth = depth 
        self.planet = planet
        self.avg_point_dist = depth
        #get no of the points
        no_points = int((end_arc - start_arc)/(0.05*np.pi))
        angle_diff = (end_arc - start_arc)/no_points
        #create points 
        self.points = [[depth, start_arc + i*angle_diff] for i in range(0,no_points)]
        
    def liquid_pos(self, planet_list):
        for point in self.points:
            point.dist = absval(point.pos - self.planet.pos)
            #reset points forces 
            point.g_force = np.zeros(2)
            point.g_force_abs = 0.0
            #get new forces 
            for planet in planet_list:
                point_dist_vec = point.pos - planet.pos
                point.g_force += (point_dist_vec/absval(point_dist_vec))*planet.mass/(absval(point_dist_vec)**2)
            point.g_force_abs = absval(point.g_force)
        #get the average point distance to the planet 
        self.avg_point_dist = np.mean([point.g_force_abs for point in self.points])
        #get the diff to avg point
        for point in self.points:
            print('t',self.avg_point_dist, point.g_force_abs)
            point_dist_diff = point.g_force_abs/self.avg_point_dist
            print(point_dist_diff, type(point_dist_diff),type(point.pos))
            point.pos[0] = point_dist_diff*point.pos*self.depth
...
class obj_planet:
    def __init__(self, pos, vel, mass, geom):
        ...
        self.grav_flag = 0
        #add liquid 
        self.liquid = liquid(0.1*np.pi, 0.9*np.pi, 10, self)
...
p2 = obj_planet(np.array([-400.0, 100.0]), np.array([0.0,0.0]), 150.0, geom)
p3 = obj_planet(np.array([120.0, 100.0]), np.array([0.0,2.6]), 50.0, n_geom)
p3.grav_flag = 1
p1 = obj_planet(np.array([-60.0, 100.0]), np.array([0.0,0.0]), 150.0, geom)
p1.grav_flag = 0
obj4 = obj_moveable(np.array([-54.0,-120.0]), np.array([19.0,0.0]), 2.0, [np.array([0.0,0.1])])
