...
class obj_moveable:
    ...
    def move_force(self):
        ...
        if self.move_keys[2] == 1:
            ...
            self.vel += -3*self.down 
            print('dpwn ',self.down, self.vel)
            #print(t)
    ...
    def screen_step(self):
        ...
        for planet in self.planet_list:
            ...
            if absval(planet.screen_pos) <= np.sqrt((self.window_x_size*2)**2 + (self.window_y_size*2)**2):
                ...
                pg.draw.line(screen,(255, 0, 0),obj.screen_pos,obj.screen_pos + obj.n_force*60)
                pg.draw.line(screen,(255, 0, 0),planet.screen_pos,planet.screen_pos + planet.force*600)
    ...
    def step(self):
        for obj in self.object_list:
            ...
            for planet in self.planet_list:
                ...
                if obj_dist < 1.2*(planet.max_rad):
                    ...
                    for point in point_list:
                        ...
                        if abs(rad) >= abs(obj_dist_pol[0]):                  
                            ...
                            #if smaller than radius amend position 
                            #if rad > obj_dist_pol[0]:
                            #    obj_dist_pol = (rad,obj_dist_pol[1])
                            #    obj.pos = pol2cart(obj_dist_pol[0],obj_dist_pol[1]) + planet.pos                                
                            ...
                            #add velocity speed here
                            obj.vel = (1-point[3])*obj.vel
                            print('vel-------------------------',obj.vel, obj.down, obj.vel/obj.down)
                            if absval(obj.vel) < 0.01:
                                obj.vel = np.zeros(2)
                            ...
                            if point[2] == 'arc':
                                ...
                                if abs(rad) > abs(obj_dist_pol[0]):
                                    normal_force -= (rad - obj_dist_pol[0])*obj_dist_vec/absval(obj_dist_vec)
                                ...
                                #vel_comp = -1*vec_comp(obj.vel, obj_dist_vec/absval(obj_dist_vec))*(obj_dist_vec/absval(obj_dist_vec))
                                vel_comp = -1*vec_comp(obj.vel, obj.down/absval(obj.down))*(obj.down/absval(obj.down))
                            
                                if round(vel_comp[0],2) != round(-1*obj.vel[0],2) and round(vel_comp[1],2) != round(-1*obj.vel[1],2):
                                    print(vel_comp[0] + obj.vel[0],vel_comp[1] + obj.vel[1])
                                    print('evl 1 ------',obj.vel,vel_comp)
                                    #print(t)
                                    #obj.vel = np.zeros(2)
                                    #obj.vel += -1*vec_comp(obj.vel, obj_dist_vec/absval(obj_dist_vec))*(obj_dist_vec/absval(obj_dist_vec))
                                normal_force += vel_comp
                ...
                #obj.force += obj.vel 
            #update the jump force
            if obj.contact_flag == 0 or obj.action_keys[2]==1:
                if absval(obj.vel) != 0:
                    obj.vel = obj.vel + obj.g_force
...
p2 = obj_planet(np.array([-200.0, -60.0]), np.array([0.0,0.0]), 1.0, n_geom)
p3 = obj_planet(np.array([-200.0, 200.0]), np.array([0.0,0.0]), 5.0, n_geom)

p1 = obj_planet(np.array([-60.0, 100.0]), np.array([0.0,0.0]), 70.0, geom)

obj4 = obj_moveable(np.array([40.0,20.0]), np.array([0.0,7.0]), 2.0, [np.array([0.0,0.1])])
