...
class game:   
    ...
    def load_sector(self, coords): 
        if list(coords) not in self.sector_load_list:
            ...
            for obj in new_data['object_list']:
                if obj['obj_type'] == 'obj':
                    new_obj_list.append(obj_moveable(obj['obj_id'], obj['obj_type'], obj['pos'], obj['vel'], obj['mass'], obj['geom']))
                if obj['obj_type'] == 'npc':
                    npc = NPC(obj['obj_id'], obj['obj_type'], obj['pos'], obj['vel'], obj['mass'], obj['geom'], [], [1,1,1])
                    npc.home_planet = self.planet_list[obj['locations']['home'][0]]
                    npc.home = self.planet_list[obj['locations']['home'][0]].planet_obj_list[obj['locations']['home'][1]]
                    npc.work_planet = self.planet_list[obj['locations']['work'][0]]
                    npc.work_location = self.planet_list[obj['locations']['work'][0]].planet_obj_list[obj['locations']['work'][1]]
                    new_obj_list.append(npc)
    ...
    def draw_planet(self, planet):
        ...
        for planet_obj in planet.planet_obj_list:
            ...
            if planet_obj.img != None:
                ...
                self.screen.blit(planet_obj_display_img, (planet.screen_pos + np.floor(planet_obj.surface_vector*self.window_scale_val)) - np.floor(np.array([planet_objRect.centerx,planet_objRect.centery])) + np.floor((surface_vector/absval(surface_vector))*0.5*planet_obj_height))
                ...
                if planet_obj.obj_type == 'display':
                    if planet_obj.state == 1:
                        ...
                        self.screen.blit(text_img_display, (planet.screen_pos + np.floor(np.array(pol2cart(planet_obj.rel_pos[0],planet_obj.rel_pos[1]))*self.window_scale_val) - np.floor(np.array([planet_objRect.centerx,planet_objRect.centery])) + np.floor(np.array([text_objRect.centerx,text_objRect.centery]))))# + (surface_vector/absval(surface_vector))*0.7*planet_obj_height))
    ...
    def draw_obj(self, obj):
        ...
        if absval(obj.screen_pos) <= np.sqrt((self.window_x_size*2)**2 + (self.window_y_size*2)**2):
            ...
            obj.screen_pos = ((obj.pos - self.window_pos)*self.window_scale_val) + np.array([self.window_x_size/2,self.window_y_size/2])
            #draw the object
            if obj.obj_type == 'player' or (obj.obj_type == 'npc' and obj.inside_flag == self.player.inside_flag):
                ...
                pg.draw.line(self.screen,(255, 0, 255),obj.screen_pos,obj.screen_pos + obj.n_force*400)
            ...
            if obj.obj_type in ['npc','player']:
                if obj.conv_flag == 1:
                    if obj.obj_type == 'player':
                        pg.draw.rect(self.screen, (202, 232, 252), pg.Rect(np.floor(obj.screen_pos[0]) + 30, np.floor(obj.screen_pos[1]) + 30, 120, self.fontsize*len(obj.conv_text_options) + 6), 0, 3)
                        font = pg.font.SysFont(None, self.fontsize)
                        for text_option_ind, text_option in enumerate(obj.conv_text_options):
                            if text_option_ind != obj.conv_keys[3]:
                                text_img = font.render(text_option, True, (15, 15, 15))
                            else:
                                text_img = font.render(text_option, True, (85, 85, 85))
                            self.screen.blit(text_img, (obj.screen_pos + 30 + np.array([0,self.fontsize-2])*text_option_ind))
                        if obj.conv_phrase != None:
                            text_img = font.render(obj.conv_phrase, True, (85, 85, 85))
                            self.screen.blit(text_img, (obj.screen_pos + 30 + np.array([0,-1*self.fontsize-2])))
                    else:
                        pg.draw.rect(self.screen, (252, 243, 202), pg.Rect(np.floor(obj.screen_pos[0]) - 150, np.floor(obj.screen_pos[1]) + 33, 120, 30), 0, 3)
                        font = pg.font.SysFont(None, self.fontsize)
                        if obj.conv_phrase != None:
                            text_img = font.render(obj.conv_phrase, True, (85, 85, 85))
                            self.screen.blit(text_img, (obj.screen_pos + np.array([-150, 33]) + np.array([0,0.5*self.fontsize-2])))                        
    def screen_step(self):
        if self.player.inside_flag == 0:
            if self.player.prev_inside_flag == 1:
                ...
                print('t')
                print('window pos',self.window_pos, self.player.pos, self.player.outside_pos)
            ...
            for obj in self.object_list:
                self.draw_obj(obj) 
                try:
                    pg.draw.circle(self.screen, (255,0, 0), (obj.home_planet.screen_pos + np.array(pol2cart(obj.home.rel_pos[0],obj.home.rel_pos[1]))*self.window_scale_val), 3)
                except:
                    pass
                try:
                    pg.draw.line(self.screen,(255, 0, 255),obj.screen_pos,obj.screen_pos + obj.npc_dist_vec*self.window_scale_val)
                except:
                    pass
        else:
            try:
        ...
        if obj_dist < 1.2*(planet.max_rad):
            ...
            if rad_list != []:
                ...
                or abs(rad) >= abs(cart2pol(-1*(obj_dist_vec - obj.g_force)[0], (obj_dist_vec - obj.g_force)[1])[0])):                  
                    ...
                    if point[2] == 'arc':
                        obj.m_force += obj.m_speed*(self.orth_vec(obj.down))*self.window_scale_val
                    ...
                    if point[2] == 'line':
                        ...
                        orth_down_vec = np.array([-1*obj_dist_vec[1],obj_dist_vec[0]])
                        
                        #add speed force 
                        obj.m_force += self.window_scale_val*0.8*obj.m_speed*(np.array([-1*new_point_line[0],new_point_line[1]])/absval(orth_vec))
                        ...
                        #test for mu 
                        if np.sign(obj.force[0]) == np.sign(obj_dist_vec[0]) or np.sign(obj.force[1]) == np.sign(obj_dist_vec[1]):
                            basic_normal_force = -1*vec_comp(obj.force, orth_vec)*(orth_vec/absval(orth_vec))
                            #get component relative to orthogonal of down direction
                            basic_normal_force_comp = vec_comp(basic_normal_force, orth_down_vec/absval(orth_down_vec))
                            
                            #get proportion of normal force 
                            rad_diff = point[1][0] - point[0][0]                                     
                            delta_angle = 4*(point[1][1] - point[0][1])/np.pi
                            if delta_angle <= 0.15 and rad_diff > 10:
                                normal_mod = delta_angle/0.15
                            else:    
                                normal_mod = 1
                            
                            #remove the movement depedning on angle and mu 
                            if abs(basic_normal_force_comp) < point[3]:
                                basic_normal_force -= normal_mod*basic_normal_force_comp*(orth_down_vec/absval(orth_down_vec))
                            else:
                                basic_normal_force -= normal_mod*np.sign(basic_normal_force_comp)*(point[3])*(orth_down_vec/absval(orth_down_vec))
                                                        
                            normal_force += basic_normal_force
    ...
    def step(self):
        for obj in self.object_list:
            ...
            #update npc obj step
            if obj.obj_type in ['npc']:
                obj.npc_step()
                print(111,obj.physical_state)
                print(obj.emotion_state) 
            ...
            if self.player.inside_flag == 0:
                ...
                if self.room_obj == None:
                    ...
                    self.prev_window_scale_val = float(self.window_scale_val + 0.01)
            ...
            #obj inside flag
            obj.prev_inside_flag = obj.inside_flag
        ...
        for obj in self.object_list:          
            ...
            #obj conv step
            if obj.obj_type in ['npc', 'player']:
                obj.npc_conv(self.object_list, 40.0)
                #update prev conv keys 
                obj.prev_conv_keys = obj.conv_keys.copy()
            ...
            for planet in self.planet_list:
                ...
                for planet_obj in planet.planet_obj_list:
                    ...
                    if planet_obj.obj_type == 'cannon' and planet_obj.state == 2:
                        fire_obj = obj_moveable(len(self.object_list) + 1, 'obj', pol2cart(planet_obj.rel_pos[0],planet_obj.rel_pos[1]) + planet.pos, 20*planet_obj.dir, 0.1, [np.array([0.0,0.1])])
