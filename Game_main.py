...
player_main = player(101, 'player', np.array([0.0,0.0]), np.array([0.0,0.0]), 2.0, [np.array([0.0,0.1])], [], [1,1,1])
#game_main = game(player_main, "input_data_7_2.json", "stars_data.json", screen)
#game_main = game(player_main, "input_data_demo_solar_system.json", "stars_data.json", screen)
...
while running:
    ...
    if game_main.player.contact_flag == 1 or game_main.player.water_contact_flag == 1:
        ...
        # if keys[pg.K_ENTER]:
        #     game_main.player.move_keys[3] = 1
        # else:
        #     game_main.player.move_keys[3] = 0        
    ...
    #     game_main.window_scale_val += 0.01
    if keys[pg.K_z]:
        game_main.window_scale_val = (0.9)*game_main.window_scale_val
    if keys[pg.K_x]:
        game_main.window_scale_val = (1/0.9)*game_main.window_scale_val       
    ...
    #action for conv
    if keys[pg.K_h]:
        game_main.player.conv_keys[0] = 1
    if keys[pg.K_j]:
        game_main.player.conv_keys[0] = 0  
    if game_main.player.conv_flag == 1:
        if keys[pg.K_a]:
            game_main.player.conv_keys[1] = 1
        else:
            game_main.player.conv_keys[1] = 0
        if keys[pg.K_d]:
            game_main.player.conv_keys[2] = 1     
        else:
            game_main.player.conv_keys[2] = 0
        if keys[pg.K_RETURN]:
            game_main.player.conv_keys[4] = 1   
        else:
            game_main.player.conv_keys[4] = 0
    #main game step 
    game_main.load_step()
